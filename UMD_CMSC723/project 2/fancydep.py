...
def train(train_data, lr=1, n_iter=10):
    ...
    for i in range(n_iter):
        ...
        for j in range(len(sentences)):
            ...
            ## get actions and relations
            relations = [] # used for reduce
            while len(buffer) != 0:
                # len(stack) != 1 and 
                # left
                ...
                # right
                    ...
                    action = 'R'
                    relations.append(str(stack[-1]) + '_' + str(buffer[0]))
                    stack.append(buffer.pop(0))
                # reduce
                elif len(stack) >=2 and ( (str(stack[-2]) + '_' + str(stack[-1])) in relations):
                    action = 'D'
                    stack.pop(-1)
#                    for b in buffer[1:]:
#                        if sen[b][6] == stack[-1]:
...
#                    restriction = False # back to default
            ...
            ## predict
            delta = {'L_bias': 0., 'R_bias': 0., 'S_bias': 0., 'D_bias':0.} # cached weights and bias
            ...
            for config in range(len(actions)):
                ...
                f1_D = 'D_stack_top_word=' + sen[ stack[-1] ][1]
                ...
                f2_D = 'D_buffer_head_word=' + sen[ buffer[0] ][1]
                ...
                f3_D = 'D_stack_top_POS=' + sen[ stack[-1] ][3]
                ...
                f4_D = 'D_buffer_head_POS=' + sen[ buffer[0] ][3]
                ...
                f5_D = 'D_pair_word=' + sen[ stack[-1] ][1] + '_' + sen[ buffer[0] ][1]
                ...
                f6_D = 'D_pair_POS=' + sen[ stack[-1] ][3] + '_' + sen[ buffer[0] ][3]
                # feature list
                f_list = [f1_L, f1_R, f1_S, f1_D, f2_L, f2_R, f2_S, f2_D,
                          f3_L, f3_R, f3_S, f3_D, f4_L, f4_R, f4_S, f4_D,
                          f5_L, f5_R, f5_S, f5_D, f6_L, f6_R, f6_S, f6_D]
                ...
                D_weights = np.array([features[L] for L in f_list if L[0] == 'D'])
                ...
                D_dot = np.dot(np.ones((1,6)), D_weights) + features['D_bias']
                
                pred_action = np.argmax([L_dot, R_dot, S_dot, D_dot])
                pred_action = ['L', 'R', 'S', 'D'][pred_action]
                
                # reduce?
                if pred_action == 'D' and len(stack) >= 2 and ( (str(stack[-2]) + '_' + str(stack[-1])) in relations):
                    pred_action = 'D'
                else:
...
def predict(test_data, features):
    ...
    for sen in sentences:
        ...
        # features
        relations = [] # used for left-arc and reduce
        while len(stack) > 0 and len(buffer) > 0:            
            ...
            D_weights = np.array([feat_copy[L] for L in f_list if L[0] == 'D'])
            ...
            D_dot = np.dot(np.ones((1,6)), D_weights) + feat_copy['D_bias']
        ...
        ## change head=-1 to head=most_frequent_head
        ## result
        res = sen
        heads_cp = heads.copy()
        for h in heads_cp:
            if h == -1:
                heads_cp.remove(h)        
        freq = mode(heads_cp)[0][0]
        for ix in range(len(res)):
            res[ix][6] = heads[ix]
        for ix_2 in range(len(res)):
            if res[ix_2][6] == -1:
                res[ix_2][6] = freq
...
#         ## train
#         features = {'L_bias': 0., 'R_bias': 0., 'S_bias': 0., 'D_bias':0.}
#         train('en.tr', lr = i, n_iter = j)
#         
#         ## predict
#         write(predict('en.dev', features), 'result')
#         
#         ## evaluation
#         print('\nlearing rate = ', i, ', iteration = ', j)
#         eval('en.dev', 'result')
#         
#         ## remove file
#         os.remove('result')
